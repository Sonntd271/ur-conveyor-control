...
def vs_recv():
        ...
        while  v_coor == [0,0,0] :
                while v_data == '':
                        ...
                        v.send (b'start!')
                        print('bytes sent, waiting for recv...')
                        v_data = v.recv(20)
                        print(v_data)
                        print('received')
...
# 0.235
        robot_home()
        time.sleep(delay)
        v_x, v_y, v_rz = vs_recv() # cm, cm, deg
        print(v_x, v_y, v_rz)

        gripper_to_cam = 11.5 # cm
        # v_x += gripper_to_cam
        v_x = 17.5

        print("--- Khan ---")
        print(v_x, v_y, v_rz)
        robot_moveTCPmode(x=v_x, y=v_y, rz=0, mode=0) # Move gripper to top of box
        time.sleep(delay)
        robot_moveTCPmode(x=v_x, y=v_y, rz=0, mode=1) # Move gripper down
        time.sleep(delay)

        grip_control(255)
        time.sleep(delay)
        robot_moveTCPmode(x=v_x, y=v_y, rz=0, mode=2) # Move gripper to top of box
        time.sleep(delay)
        robot_moveTCPmode(x=v_x, y=v_y, rz=0, mode=1) # Move gripper down
        time.sleep(delay)
        grip_control(0)
